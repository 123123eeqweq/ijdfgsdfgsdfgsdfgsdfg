...
const polygonCandleSchema = new mongoose.Schema({
  pair: {
    type: String,
    required: true,
    index: true
...
polygonCandleSchema.index({ pair: 1, startTime: 1 }, { unique: true });

// Индекс для сортировки по времени
polygonCandleSchema.index({ startTime: -1 });

// 🔥 КРИТИЧНО: Индекс для запросов с фильтром isClosed
// Оптимизирует запросы: find({ pair, isClosed: true }).sort({ startTime: -1 })
polygonCandleSchema.index({ pair: 1, isClosed: 1, startTime: -1 });

const PolygonCandle = mongoose.model('PolygonCandle', polygonCandleSchema);

module.exports = PolygonCandle;
